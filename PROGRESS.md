...
### 24. **🚨 T8.5e: alarm sidecar live trigger 通過 E-stop 驗證（2026-06-05）**

T8.4 寫的 alarm sidecar（`_await_motion_complete` 裡 sync_task vs alarm_task 賽跑、輸的取消、贏的若是 alarm 就印 `⚠ ALARM` + 主動 `GetErrorID()`）在實機 fire 過了。

**觸發**：`move_l 199.7 0 -28.9 0 1`（speed=1 慢速 10mm 行程）→ motion 中約 1 秒按下控制器物理紅色 E-stop 按鈕。

**workbench 輸出**：
```
move_l: target=(199.7,0.0,-28.9,0.0) speed=1
⚠ move_l: ALARM — mode=9 err=True
  GetErrorID: GetErrorIDResult(error_id=0, controller_errors=(85,), servo_errors=((), (), (), ()))
```

**Error 85 對照** `reference/.../alarm_controller.json:689`:
> "Hardware emergency stop button press or software emergency stop trigger"

完全對應實際操作。

**這條 finding 補了 PROGRESS finding 7（錯誤訊號三條通道）的細節**：

1. **30004 feedback 推 alarm frame latency 量級 < 100ms** — 按 E-stop 跟看到 print 之間使用者體感「即時」，遠快於 Sync 的 20s timeout。
2. **`robot_mode` enum 變化不被 RobotState 的 deadband 擋** — deadband 只過濾浮點欄位（pose/joint 抖動），enum int + bool 一變就 fire callback；alarm sidecar 賴這個及時性。
3. **`asyncio.wait(FIRST_COMPLETED)` 對 thread-backed sync_task 的取消有效** — alarm_task 贏了之後 `sync_task.cancel()` 正確識別 pending；Sync() 的 thread 雖然會繼續跑到 controller 回應為止，但 future 被 GC、不阻塞 cmd_move_l return。
4. **`GetErrorID()` 巢狀 list parser 對齊 SDK 規範** — 回應 `{[[85],[],[],[],[]]}` 五層 list（controller + 4 個 servo）正確解成 `controller_errors=(85,)` + `servo_errors=((), (), (), ()) `。`error_id=0` 是 query 本身 ack（query 成功 ≠ 沒錯誤；錯誤資料在巢狀 list 裡）。
5. **E-stop 自動 disable servo** — 復原必須 `clear` + `enable` 兩步：單 `clear` 後 `is_enabled=False`，下次 motion 會在 `_check_motion_preconditions` 被擋。`clear` 只清 error 旗標，**不**重啟 servo。

**為什麼這條測試一定要走 E-stop（或推手臂）而非「下一條會 alarm 的指令」**：

safety polygon margin 設計上**比 controller 真實 alarm 邊界更嚴格**（J3-J2 在 `59.95°` 擋下 vs controller alarm 在 `60°`），保證 workbench 永遠先擋。代價：沒有任何 joint 目標能「通過 polygon 又讓 controller alarm」——範圍分離，中間沒重疊。所以 alarm sidecar 的 print path 不可能由「下一條 move 指令」激活，必須靠跟目標脫鉤的外部事件（E-stop、collision、payload 突變、斷電）。

這也意味著：**production 場景 alarm sidecar 通常不會 fire**，它的角色是「最後一道意外捕捉網」，平常靠 safety gate（更上游）+ Sync clean completion（更下游）兩條走完。Unit test 用 mock 覆蓋了 print path 的正確性，T8.5e 補的是「mode=9 訂閱 + asyncio race + GetErrorID parser」三件事在真實 controller 上的整合對應。

**對未來實作的意義**：
- UI（段 2）可以信任 `RobotState.subscribe()` 對 mode 變化的即時性，無需另設 polling
- controller phase（T10）的狀態機可以用「`error_id=0` 不代表沒錯誤」這條釐清 GetErrorID 的角色——它是錯誤明細 query，不是「有沒有錯」的判斷（後者看 feedback `has_error`）
- 拾取場景錯誤恢復 sequence 固定為 `clear → enable → 重新 set_payload(load,cx,cy,cz)`（finding 21）

### 25. **✅ T9: 4-pose multi-DOF stress demo 通過 125 iters（2026-06-05）— 段 1 完成訊號**

T8.5 motion path 用 `workbench demo_loop` 跑 stress demo 驗證 Phase 5 「真實工作流量」下的穩定性。

**Test scope**: 4-pose 序列 `T9_POSE_SEQUENCE`（factory neutral / 前右+R30° / 前左-R30° / z=+30 抬高），speed=20，三段累計 **125 iters / ~31.25 cycles**，分三次跑驗 escalation：

| run | iters | cycles | 結果 |
|---|---|---|---|
| 2.0 conservative | 5 | 1.25 | ✓ 全綠 |
| 2.1 stress | 100 | 25 | ✓ 全綠 |
| 2.2 default 再跑 | 20 | 5 | ✓ 全綠 |

**核心觀察**：

1. **Δ30004 全程 0.00mm**。125 iters 後 final status `Δ30004=0.00mm`，**FK↔feedback 對齊不累積漂移**——MovL 進佇列→Sync 等→feedback 推下一幀，joint 數字回到 IK 預期、FK 算出 (230.0, 0.0, 30.0, 0.0) 跟 target 完全一致。這條反證了「Phase 5 motion path 不會因為長時間 stress 而 drift」對 T10 controller `move_to(...)` API 是 safe foundation。

2. **無 ALARM / REJECT / queue error / Sync timeout**。125 個 `_await_motion_complete` 都走 sync_task 贏的路徑（alarm sidecar 訂閱被 cancel），跟 unit test 模型對稱。

3. **Pose[3] 抬高位置 hold 時有輕度 servo 嗡聲**（跟 finding 21 phantom load whine **不同**）：
   - **這次**：enable 0-param（無 load 宣告）、嗡聲只在「motion 完成、static hold pose[3]」之後出現
   - **finding 21**：enable load=0.5（無實際負載）、嗡聲 enable 一執行就**連續**到 disable
   - 機制：pose[3] = (230, 0, 30, 0) 對應 joints ≈ (0, -10, 30, 0)，J2=-10° 後臂前傾、J3=30° 前臂抬起 → 重力對 J2/J3 軸有靜態力矩 → servo 持續輸出小電流維持位置 → 高頻電流調變產生可聞高頻嗡。Factory pose (J2=0 鉛直) 不嗡因為 J2 力矩零。
   - **意義**：未來看到嗡聲要判時間 pattern：「enable 一啟動就嗡 → phantom load (finding 21)」vs「只在非鉛直 pose hold 才嗡 → 重力 servo hold (這條)」。兩條時間特徵不同，可作為快速 diagnostic。

**對段 1 Phase 5 motion 原語的意義**：T8.4 寫的 motion path（safety gate → 入隊 → Sync wait → alarm sidecar）在「stress 125 iters / 多 DOF / 多 pose」場景驗為**生產就緒**。`_move_l_internal` refactor 後的 bool return 介面 + op_desc print prefix 在 demo_loop 用了 125 次都對應正確，T10 controller `move_to(pose, user=, tool=, speed=)` 可以放心建在這上面。

**對段 2 UI 的意義**：UI 的「點選 pose → 送 move_l」鏈路在這個 stress level 不會卡。8ms feedback cadence + Δ 不漂移 → UI 顯示 robot 即時位置不會跟實機脫節。
